...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	override function create()
	{
		...
		else if (FlxG.save.data.autosave != null) {
	...
	*/
	...
	private function saveLevel()
	{
		...
		// MIND YOU I DID ALL THIS TO RECOVER POST STORM
		var data:String = Json.stringify(json, "\t");
		var songFormatted:String = _song.song.replace(' ', '-');

		#if MODS_ALLOWED
		var path:String = Paths.modFolders('data/$songFormatted/');
		#else
		var path:String = Paths.getPreloadPath('data/$songFormatted/');
		#end
		...
		var fullPath:String = '$path' + Paths.formatToSongPath(_song.song) + '.json';

		if ((data != null) && (data.length > 0))
		{
			try {
				File.saveContent(fullPath, data.trim());
				trace('Chart saved to "$fullPath"');
				openSubState(new Prompt('Chart saved to "$fullPath"', 0, null, null, ignoreWarnings));
			} catch(e) {
				trace('Chart cannot be saved: \n$e');
				openSubState(new Prompt('Chart could not be saved to "$fullPath"', 0, null, null, ignoreWarnings));
			}
