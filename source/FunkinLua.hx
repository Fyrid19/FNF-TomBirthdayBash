...
class FunkinLua {
	...
	public function new(script:String) {
		...
		Lua_helper.add_callback(lua, "makeLuaVideo", function(tag:String, videoPath:String, x:Float, y:Float, ignorePause:Bool = false) {
			tag = tag.replace('.', '');
			resetVideoTag(tag);
			var video:ModchartVideo = new ModchartVideo(x, y);
			if(videoPath != null && videoPath.length > 0) {
				...
				video.bitmap.onEndReached.add(function() {
					PlayState.instance.callOnLuas('onLuaVideoFinished', [tag]);
				});
				video.bitmap.onPlaying.add(function() {
					PlayState.instance.callOnLuas('onLuaVideoStarted', [tag]);
				});
				video.bitmap.onStopped.add(function() {
					PlayState.instance.callOnLuas('onLuaVideoStopped', [tag]);
				});
				video.bitmap.onPaused.add(function() {
					PlayState.instance.callOnLuas('onLuaVideoPaused', [tag]);
				});
				video.load(Paths.video(videoPath));
			}
			video.antialiasing = ClientPrefs.globalAntialiasing;
			video.videoPath = videoPath;
			video.ignorePause = ignorePause;
			PlayState.instance.modchartVideos.set(tag, video);
			video.active = true;
		});

		Lua_helper.add_callback(lua, "addLuaVideo", function(tag:String) {
			if(PlayState.instance.modchartVideos.exists(tag)) {
				var shit:ModchartVideo = PlayState.instance.modchartVideos.get(tag);
				if (!shit.wasAdded) {
					getInstance().add(shit);
					shit.wasAdded = true;
				}
		...
		Lua_helper.add_callback(lua, "playLuaVideo", function(tag:String) {
			if (PlayState.instance.modchartVideos.exists(tag)) {
				var videoPath = PlayState.instance.modchartVideos.get(tag).videoPath;
				PlayState.instance.modchartVideos.get(tag).play();
			}
		});

		Lua_helper.add_callback(lua, "stopLuaVideo", function(tag:String) {
			if (PlayState.instance.modchartVideos.exists(tag)) {
				PlayState.instance.modchartVideos.get(tag).stop();
			}
		});

		Lua_helper.add_callback(lua, "pauseLuaVideo", function(tag:String, stayPaused:Bool = false) {
			if (PlayState.instance.modchartVideos.exists(tag)) {
				PlayState.instance.modchartVideos.get(tag).togglePaused();
				PlayState.instance.modchartVideos.get(tag).stayPaused = stayPaused;
			}
		});

		Lua_helper.add_callback(lua, "restartLuaVideo", function(tag:String) {
			if (PlayState.instance.modchartVideos.exists(tag)) {
				PlayState.instance.modchartVideos.get(tag).bitmap.position = 0;
				PlayState.instance.callOnLuas('onLuaVideoRestart', [tag]);
			}
		});

		Lua_helper.add_callback(lua, "changeLuaVideoVolume", function(tag:String, vol:Int) {
			if (PlayState.instance.modchartVideos.exists(tag)) {
				PlayState.instance.modchartVideos.get(tag).bitmap.volume = vol;
			}
		});
		...
		Lua_helper.add_callback(lua, "removeLuaVideo", function(tag:String, destroy:Bool = true) {
			...
			var pee:ModchartVideo = PlayState.instance.modchartVideos.get(tag);
			if(destroy) {
				pee.kill();
			}
		...
		Lua_helper.add_callback(lua, "luaVideoExists", function(tag:String) {
			return PlayState.instance.modchartVideos.exists(tag);
		});
...
class ModchartText extends FlxText
{
	...
	public function new(x:Float, y:Float, text:String, width:Float)
	{
		...
		setFormat(Paths.font("tomgles.tff"), 16, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
...
class ModchartVideo extends FlxVideoSprite {
	public var wasAdded:Bool = false;
	public var stayPaused:Bool = false;
	public var ignorePause:Bool = false;
...
class DebugLuaText extends FlxText
{
	...
	public function new(text:String, parentGroup:FlxTypedGroup<DebugLuaText>, color:FlxColor) {
		...
		setFormat(Paths.font("tomgles.tff"), 20, color, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		scrollFactor.set();
		borderSize = 1;
	}
